...
function main() {
    ...
    if (env.profiling) {
        alert("层数已计入 " + profiler.format(countDuration), "调试信息");
    }
    ...
    if (BATCH_OPERATION || showDialog() === 1) {
        ...
        if (env.profiling) {
            alert("收集的层数" + profiler.format(collectionDuration), "调试信息");
        }
        ...
        if (foldersOk === true) {
            ...
            if (env.profiling) {
                message += "\n\nn导出函数执行耗时 " + profiler.format(collectionDuration) + " + " + profiler.format(exportDuration) + "";
            }
            if (count.error) {
                message += "\n\n部分图层导出失败！（是否有多个同名图层？）";
            }
...
function createProgressBar() {
    ...
    var rsrcFile = new File(env.scriptFileDirectory + "/" + encodeURI("Export Layers To Files (Fast)-progress_bar-chinese.json"));
    ...
    try {
        ...
        alert("进度条资源已损坏！请重新下载包含所有文件的脚本。", "Error", true);
        return false;
    }
...
function showDialog() {
    ...
    try {
        ...
        alert("打开对话框时出错！请提交问题并尝试旧版本。", "错误", true);
        return false;
    }
...
function bootstrap() {
    function showError(err) {
        alert(err + ':在 ' + err.line + ' 行', '脚本错误', true);
    }
    ...
    try {
        ...
        if (env.version < 9) {
            alert("CS2 之前的 Photoshop 版本不受支持！", "Error", true);
            return "cancel";
        }
...
function collectLayersAM(progressBarWindow) {
    ...
    if (layerCount == 0) {
        ...
        if (progressBarWindow) {
            ...
            showProgressBar(progressBarWindow, "收集图层...可能需要几秒钟。", (layerCount + FEW_LAYERS) / FEW_LAYERS);
        }
...
function countLayersAM(progressBarWindow) {
    ...
    if (layerCount == 0) {
        ...
        if (progressBarWindow) {
            // The layer count is actually + 1 if there's a background present, but it should be no biggie.
            showProgressBar(progressBarWindow, "计算层数...可能需要几秒钟。", (layerCount + FEW_LAYERS) / FEW_LAYERS);
        }
...
function loadResource(file) {
    var rsrcString;
    if (!file.exists) {
        alert("导出对话框的资源文件‘ '" + file.name + "' 丢失！请下载此脚本附带的其余文件。", "错误", true);
        return false;
    }
    try {
        file.open("r");
        ...
        alert("无法读取资源文件 '" + file.name + "'!\n\n原因: " + error + "\n\n请检查它是否可供读取，如果文件损坏，请重新下载。", "错误", true);
        return false;
    }
...
function makeMainDialog() {    
    ...
    txtDestination.helpTip = "谷歌翻译：文件的保存位置"; 
    ...
    radioSelected.helpTip = "谷歌翻译：仅导出选定的组。必须选择一个组才能启用。"; 
    radioSelected.text = "选择组"; 
    ...
    cbVisibleOnly.helpTip = "谷歌翻译：是否仅导出可见图层"; 
    cbVisibleOnly.text = "仅可见"; 
    ...
    cbIgnorePrefix.helpTip = "谷歌翻译：忽略以...开头的层"; 
    cbIgnorePrefix.text = "忽略以“...”开头的层（如：!）"; 
    ...
    txtIgnorePrefix.helpTip = "谷歌翻译：要匹配的前缀"; 
    ...
    var ddNameAs_array = ["使用图层名称（删除扩展名）","使用图层名称（保留扩展名）","使用图层和父组名称","使用索引降序","使用索引升序"]; 
    ...
    cbDelimiter.helpTip = "谷歌翻译：是否在单词之间使用自定义分隔符"; 
    ...
    txtDelimiter.helpTip = "谷歌翻译：单词之间使用的分隔符"; 
    txtDelimiter.text = "_"; 
    ...
    var ddLetterCasing_array = ["保留","小写","大写"]; 
    ...
    txtPrefix.helpTip = "谷歌翻译：每个图层名称前都会添加前缀"; 
   ...
   // txtSuffix.helpTip = "Suffix will be added after every layer name"; 
    txtSuffix.helpTip = "谷歌翻译：每个图层名称后都会添加后缀"; 
   ...
   // btnRun.helpTip = "Runs the script with the selected settings"; 
    btnRun.helpTip = "谷歌翻译：使用选定的设置运行脚本"; 
    btnRun.text = "运行"; 
    ...
    btnCancel.helpTip = "谷歌翻译：关闭对话框并且不保存任何更改"; 
    btnCancel.text = "取消"; 
    ...
    btnSaveAndCancel.helpTip = "谷歌翻译：关闭对话框但保存所做的任何更改"; 
    ...
    cbOverwriteFiles.helpTip = "谷歌翻译：如果选中，则将覆盖同名的现有文件。否则，将创建唯一的副本。"; 
    cbOverwriteFiles.text = "覆盖现有文件"; 
    ...
    cbSilent.helpTip = "谷歌翻译：如果选中，运行时将没有进度条和成功确认。"; 
    cbSilent.text = "静默运行"; 
    ...
    cbGroupsAsFolders.helpTip = "谷歌翻译：组和子组保存为目录。"; 
    cbGroupsAsFolders.text = "组作为文件夹"; 
    ...
    cbTopGroupsAsFolders.helpTip = "谷歌翻译：组以目录形式保存。嵌套组中的图层将保存在其最顶层的组中。"; 
    cbTopGroupsAsFolders.text = "顶级组作为文件夹"; 
    ...
    cbTopGroupsAsLayers.helpTip = "谷歌翻译：顶级组将在导出之前将其所有子组合并为一个图层"; 
    cbTopGroupsAsLayers.text = "将组合并为图层"; 
   ...
   // cbForeground.helpTip = "The top layer will be used as a foreground in every export."; 
    cbForeground.helpTip = "谷歌翻译：顶层将在每次导出时用作前景。"; 
    ...
    cbBackground.helpTip = "谷歌翻译：底层将在每次导出时用作背景。"; 
    ...
    cbTrim.helpTip = "谷歌翻译：导出前是否修剪"; 
    cbTrim.text = "裁切"; 

    var ddTrim_array = ["每一层","每一层（使用 trim()方法）","合并"]; 
    ...
    cbPadding.helpTip = "谷歌翻译：导出前是否为每个图层添加填充"; 
    cbPadding.text = "填充"; 
    ...
    cbScale.helpTip = "谷歌翻译：导出前是否缩放每一层"; 
    cbScale.text = "缩放"; 
    ...
    lblPng24Matte.text = "蒙版"; 
	
    var ddPng24Matte_array = ["白色","黑色","灰色","-","背景","前景"]; 
    ...
    lblPng8ColorReduction.text = "色彩还原"; 
	
    var ddPng8ColorReduction_array = ["感知Perceptual","选择性 Selective","自适应 Adaptive","限制性（Web） Restrictive (Web)","-","黑白 Black  & White","灰度 Grayscale","苹果系统 Mac OS","微软系统 Windows"]; 
    ...
    var ddPng8Dither_array = ["无 None","扩散 Diffusion","模式 Pattern","噪声 Noise"]; 
    ...
    lblPng8Matte.text = "蒙版"; 

    var ddPng8Matte_array = ["白色","黑色","灰色","-","背景","前景"]; 
    ...
    lblPng8TransparencyDither.text = "背景透明-仿色"; 

    var ddPng8TransparencyDither_array = ["无 None","扩散 Diffusion","模式 Pattern","噪声 Noise"]; 
    ...
    var ddJpgMatte_array = ["白色","黑色","灰色","-","背景","前景"]; 
   ...
   // var ddPdfDestinationProfile_array = ["Japan Color 2001 Coated","Japan Color 2001 Uncoated","Japan Color 2002 Newspaper","Japan Color 2003 Web Coated","Japan Web Coated (Ad)","U.S. Sheetfed Coated v2","U.S. Sheetfed Uncoated v2","U.S. Web Coated (SWOP) v2","U.S. Web Uncoated v2","-","sRGB IEC61966-2.1","Adobe RGB (1998)","Apple RGB","ColorMatch RGBimage P3","ProPhoto RGB","Rec.601 NTSC Gamma 2.4","Rec.601 PAL Gamma 2.4","Rec.709 Gamma 2.4"]; 
    var ddPdfDestinationProfile_array = ["日本彩色 2001 铜版纸", "日本彩色 2001 非铜版纸", "日本彩色 2002 报纸纸", "日本彩色 2003 卷筒纸铜版纸", "日本卷筒纸铜版纸（广告）", "美国单张纸铜版纸 v2", "美国单张纸非铜版纸 v2", "美国卷筒纸铜版纸 (SWOP) v2", "美国卷筒纸非铜版纸 v2", "-", "sRGB IEC61966-2.1", "Adobe RGB (1998)", "Apple RGB", "ColorMatch RGBimage P3", "ProPhoto RGB", "Rec.601 NTSC 伽玛 2.4", "Rec.601 PAL 伽玛 2.4", "Rec.709 伽玛 2.4"]; 
   ...
   // var ddPdfDownSample_array = ["Do Not Downsample","Average Downsampling To","Subsampling To","Bicubic Downsampling To"]; 
    var ddPdfDownSample_array = ["不进行降采样", "区域平均法（Area Averaging）", "重采样至", "双三次插值（Bicubic Resampling）"]; 
    ...
    lblPdfDownSampleSizeLimit.text = "对于上面的图片"; 
    ...
    var ddTgaDepth_array = ["16 位 bit","24 位 bit","36 位 bit"]; 
    ...
    var ddBmpDepth_array = ["24 位 bit","32 位 bit","RGB 565 (16 位 bit)","ARGB 1555 (16 位 bit)","ARGB 4444 (16 位 bit)"]; 
    ...
    lblMetadata.text = "此文档包含 {0} 个图层，其中 {1} 个可见，{2} 个已选择"; 
    ...
    lblContact.add("statictext", undefined, "要获取最新版本或留下反馈，请访问：", {name: "lblContact"}); 
    ...
    lblContact.add("statictext", undefined, "（可Github搜索“Export-Layers-to-Files-Fast”）", {name: "lblContact"}); 
    lblContact.add("statictext", undefined, "感谢一众原作者（Github）：Hanna等人", {name: "lblContact"}); 
    lblContact.add("statictext", undefined, "我是TKCB-Gm，为此jsx进行了中文的翻译，如有不对敬请指正，机器翻译+部分人工校对", {name: "lblContact"}); 
